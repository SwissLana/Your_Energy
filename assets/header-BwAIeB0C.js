import{a as f,i as d}from"./vendor-CaULtfQf.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const i of r)if(i.type==="childList")for(const n of i.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function a(r){const i={};return r.integrity&&(i.integrity=r.integrity),r.referrerPolicy&&(i.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?i.credentials="include":r.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(r){if(r.ep)return;r.ep=!0;const i=a(r);fetch(r.href,i)}})();const N="https://your-energy.b.goit.study/api",de={MUSCLES:"Muscles",BODY_PARTS:"Body parts",EQUIPMENT:"Equipment"},q="your-energy-favorites",C="your-energy-quote";f.defaults.baseURL=N;async function ue(e,t,a){return(await f.get("/filters",{params:{filter:e,page:t,limit:a}})).data}async function fe(e){return(await f.get("/exercises",{params:e})).data}async function A(e){return(await f.get(`/exercises/${e}`)).data}async function U(){return(await f.get("/quote")).data}async function pe(e){return(await f.post("/subscription",{email:e})).data}async function _(e,t){return(await f.patch(`/exercises/${e}/rating`,t)).data}function Q(e){return/^\w+(\.\w+)?@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/.test(e)}function ge(){return window.innerWidth<768?8:10}function D(e,t){return(JSON.parse(localStorage.getItem("ratedExercises"))||[]).some(s=>s.exerciseId===e&&s.email===t)}function J(e,t){const a=JSON.parse(localStorage.getItem("ratedExercises"))||[];a.push({exerciseId:e,email:t}),localStorage.setItem("ratedExercises",JSON.stringify(a))}const b="/Your_Energy/assets/sprite-DcvH3MMZ.svg";function w(){return JSON.parse(localStorage.getItem(q))||[]}function M(e){localStorage.setItem(q,JSON.stringify(e))}function z(e){const t=w();t.some(a=>a===e)||(t.push(e),M(t))}function Y(e){const t=w().filter(a=>a!==e);M(t)}function F(e){return w().some(t=>t===e)}function H(e){const t=new Date().toISOString().split("T")[0];localStorage.setItem(C,JSON.stringify({date:t,quote:e}))}function K(){const e=JSON.parse(localStorage.getItem(C));if(!e)return null;const t=new Date().toISOString().split("T")[0];return e.date===t?e.quote:null}function j({name:e,filter:t,imgURL:a}){return`
  ...
  `}function ve(e){return e.map(j).join("")}function V(e,t={}){const a=typeof t=="boolean"?t:(t==null?void 0:t.isFavorite)??!1,{_id:s,name:r,rating:i,burnedCalories:n,bodyPart:v,target:m}=e,P=a?`
        <button
    ...
    <li class="exercise-card" data-id="${s}">
      ...
      <ul class="exercise-card-info">
        <li class="info-item" title="Burned calories: ${n} / 3 min">
          <span class="info-label">Burned calories:&nbsp;</span>
          <span class="info-value">${n} / 3 min</span>
        </li>
        <li class="info-item" title="Body part: ${v}">
          <span class="info-label">Body part:&nbsp;</span>
          <span class="info-value">${v}</span>
        </li>
        <li class="info-item" title="Target: ${m}">
          <span class="info-label">Target:&nbsp;</span>
          <span class="info-value">${m}</span>
        </li>
      </ul>
    </li>
  `}function me(e){return e.map(t=>V(t,!1)).join("")}function L(e){return typeof e=="string"&&e.length>0?e.charAt(0).toUpperCase()+e.slice(1):e}function G(e){const{_id:t,name:a,burnedCalories:s,bodyPart:r,target:i}=e,n=L(r),v=L(i),m=`${s} / 3 min`;return`
  ...
  `}function he(e){return e.map(G).join("")}function W(e){const t=F(e._id),a=Z(e.rating),s=t?"Remove from favorites":"Add to favorites",r=t?"icon-trash":"icon-heart";return`
  ...
  `;for(let i=1;i<=5;i++){let n;i<=Math.floor(a)?n="rgb(238, 161, 12)":i===Math.ceil(a)&&a%1!==0?n="url(#partial-star-gradient)":n="rgba(244, 244, 244, 0.2)",s+=`
      <svg width="18" height="18" aria-hidden="true" style="fill: ${n};">
        <use href="${b}#icon-add-rating-star"></use>
      </svg>
    `}return s}const p=document.querySelector("[data-loader]");function be(){p==null||p.classList.remove("is-hidden")}function ye(){p==null||p.classList.add("is-hidden")}const y=document.querySelector("[data-modal-rating]"),x=document.querySelector(".rating-form"),X=document.querySelector("[data-rating-close]"),$=document.querySelector(".live-rating-value");document.querySelectorAll('.stars-radio-group>input[type="radio"]');let S=null;function xe(){!y||!x||(X.addEventListener("click",E),y.addEventListener("click",te),x.addEventListener("submit",se),x.addEventListener("change",ae))}function ee(e){S=e,y.classList.remove("is-hidden")}function E(){$&&($.textContent="0.0"),x.reset(),y.classList.add("is-hidden"),c&&c.classList.remove("is-hidden")}function te(e){e.target===y&&E()}function ae(e){if(e.target.name!=="rating")return;const t=e.target.value;$.textContent=`${t}.0`}async function se(e){e.preventDefault();const t=new FormData(e.currentTarget),a=t.get("rating"),s=t.get("email"),r=t.get("comment");if(!a){d.warning({message:"Please select stars!",position:"topCenter"});return}if(!s.trim()||!r.trim()){d.warning({message:"Please fill out all the fields!",position:"topCenter"});return}if(!Q(s)){d.warning({message:"Please enter a valid email address!",position:"topCenter"});return}if(D(S._id,s)){d.error({message:"You have already rated this exercise!",position:"topCenter"}),E();return}const i={rate:Number(a),email:s,review:r};try{await _(S._id,i),J(S._id,s),d.success({message:"Thank you for your review!",position:"topCenter"}),E()}catch(n){console.error(n),d.error({message:"Something went wrong.",position:"topCenter"})}}const o=document.querySelector("[data-modal-root]");let c=null,u=null,h=!1,l=null,g=null;function Se(){document.addEventListener("click",re)}async function re(e){const t=e.target.closest("[data-exercise-start]");if(t)try{u=await A(t.dataset.id),ie(u)}catch(a){console.error("Failed to load exercise:",a),d.error({message:"Failed to load exercise details.",position:"topCenter"})}}function ie(e){if(!(!o||!e)){if(c){c.classList.remove("is-hidden");return}h=F(e._id),o.innerHTML=W(e),c=o.querySelector("[data-modal-backdrop]"),document.body.classList.add("no-scroll"),document.addEventListener("keydown",B),o.addEventListener("click",R),l=o.querySelector("[data-favorite-toggle]"),l&&l.addEventListener("click",O),g=o.querySelector("[data-give-rating]"),g&&g.addEventListener("click",T)}}function T(){c&&c.classList.add("is-hidden"),ee(u)}function R(e){(e.target.matches("[data-modal-backdrop]")||e.target.closest("[data-modal-close]"))&&I()}function O(){if(!l||!u)return;const e=l.querySelector("span"),t=l.querySelector("use");h?(Y(u._id),h=!1,e.textContent="Add to favorites",t.setAttribute("href",`${b}#icon-heart`)):(z(u._id),h=!0,e.textContent="Remove from favorites",t.setAttribute("href",`${b}#icon-trash`)),document.dispatchEvent(new CustomEvent("favoritesUpdated"))}function B(e){if(e.key!=="Escape")return;const t=document.querySelector("[data-modal-rating]");if(t&&!t.classList.contains("is-hidden")){const a=document.querySelector(".rating-form");a&&a.reset(),t.classList.add("is-hidden"),c&&c.classList.remove("is-hidden");return}o.innerHTML!==""&&I()}function I(){o&&(l&&(l.removeEventListener("click",O),l=null),g&&(g.removeEventListener("click",T),g=null),o.innerHTML="",u=null,c=null,h=!1,document.body.classList.remove("no-scroll"),document.removeEventListener("keydown",B),o.removeEventListener("click",R))}function ne(e){const t=document.querySelector(e),a=document.getElementById("quote-card-template");!t||!a||t.appendChild(a.content.cloneNode(!0))}async function Ee(){ne("[data-quote-card-root]");const e=document.querySelector("[data-quote-text]"),t=document.querySelector("[data-quote-author]");if(!e||!t)return;const a=K();if(a){k(a,e,t);return}try{const s=await U();H(s),k(s,e,t)}catch{e.textContent="Quote is not available now.",t.textContent=""}}function k(e,t,a){t.textContent=e.quote,a.textContent=e.author}function $e(){const e=document.querySelector("[data-scroll-up]");e&&(window.addEventListener("scroll",()=>{e.classList.toggle("is-visible",window.scrollY>400)}),e.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})}))}function oe(){const e=document.querySelectorAll(".header-nav-list .header-nav-link");if(!e.length)return;const t=s=>{const r=s.split("/").pop();return!r||r===""?"index.html":r},a=t(window.location.pathname);e.forEach(s=>{const r=s.querySelector("a");if(!r)return;const i=t(r.pathname);s.classList.toggle("active",i===a)})}function we(){oe();const e=document.querySelector("[data-menu-open]"),t=document.querySelector("[data-menu-close]"),a=document.querySelector("[data-mobile-menu]");!e||!t||!a||(e.addEventListener("click",()=>{a.classList.add("is-open"),document.body.classList.add("no-scroll")}),t.addEventListener("click",()=>{a.classList.remove("is-open"),document.body.classList.remove("no-scroll")}),a.addEventListener("click",s=>{s.target.matches("a")&&(a.classList.remove("is-open"),document.body.classList.remove("no-scroll"))}))}export{de as F,Ee as a,$e as b,Se as c,xe as d,be as e,ue as f,ve as g,ye as h,we as i,ge as j,fe as k,me as l,w as m,A as n,he as o,Y as r,pe as s};
//# sourceMappingURL=header-BwAIeB0C.js.map
